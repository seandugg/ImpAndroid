 ...
 *
 */
public class CcmFastBreathingYoungerChildSickSignDiagnosticTest extends CcmDiagnosticRuleEngineTest {
	
	private static final String BREATHS_PER_MINUTE = "51";
	private static final int PATIENT_AGE_IN_MONTHS = 8;
	
    public CcmFastBreathingYoungerChildSickSignDiagnosticTest() {
        super(); 
    }
    ...
    public void testFastBreathingSickSign() {
       ...
       assertEquals("the actual number of patient classifications does not match the expected number",
        ...
        assertEquals("incorrect treatment assessed", true, CcmRuleEngineUtilities.treatmentPresent(getPatientAssessment().getDiagnostics(), 
        		"Give oral antibiotic (cotrimoxazole adult tablet - 80/400). Give twice daily for 5 days. Help caregiver to give first dose now. Age 2 months up to 12 months: 1/2 tablet (total 5 tabs)"));																														 
